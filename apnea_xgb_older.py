# apnea_xgb_older_adult.py
# XGBoost — Sleep Apnea Risk Prediction
# Cohort: Older Adults (Age 56-79)
# Mirrors apnea_rf_older_adult.py's data/target logic exactly,
# and insomnia_xgb_young_adult.py's XGBoost config exactly.
# Saves: apnea_xgb_older_adult.pkl -> {'xgb': model, 'scaler': scaler}
...
#    Matches apnea_rf_older_adult.py filter: age > 55
# ---------------------------------------------------------------
old = df[df["age"] > 55].copy()
old.reset_index(drop=True, inplace=True)

print("\nOlder Adult cohort (56-79):")
print("  Records  :", len(old))
print("  Age range:", old["age"].min(), "to", old["age"].max())

# ---------------------------------------------------------------
# 4. BUILD BINARY APNEA TARGET
#    AASM cutoff: AHI >= 15 (at least moderate OSA) -> apnea risk = 1
# ---------------------------------------------------------------
old["apnea_risk"] = (old["apnea_risk_score"] >= 15).astype(int)

print("\nApnea risk distribution (Older Adults):")
print(old["apnea_risk"].value_counts())
print("\nClass percentages:")
print(round(old["apnea_risk"].value_counts(normalize=True) * 100, 1))

# ---------------------------------------------------------------
# 5. FEATURE SELECTION
#    Same 22 features as apnea_rf_older_adult.py
...
# ---------------------------------------------------------------
print("\n" + "=" * 60)
print("TRAINING XGBOOST — APNEA (OLDER ADULTS 56-79)")
print("=" * 60)
...
print("\n" + "=" * 60)
print("RESULTS — APNEA RISK (OLDER ADULTS 56-79)")
print("=" * 60)
...
plt.tight_layout()
plt.savefig("plots/xgb_apnea_feature_importance_older.png",
            dpi=150, bbox_inches="tight")
plt.close()
print("\n  Plot saved: plots/xgb_apnea_feature_importance_older.png")
...
plt.tight_layout()
plt.savefig("plots/xgb_apnea_confusion_matrix_older.png",
            dpi=150, bbox_inches="tight")
plt.close()
print("  Plot saved: plots/xgb_apnea_confusion_matrix_older.png")
...
ax.set_ylim([0, 1.02])
plt.tight_layout()
plt.savefig("plots/xgb_apnea_roc_curve_older.png",
            dpi=150, bbox_inches="tight")
plt.close()
print("  Plot saved: plots/xgb_apnea_roc_curve_older.png")
...
with open("apnea_xgb_older_adult.pkl", "wb") as f:
    pickle.dump(bundle, f)
print("\n  Model saved: apnea_xgb_older_adult.pkl")
...
print("SUMMARY")
print("=" * 60)
print(f"  Cohort             : Older Adults (Age 56-79)")
